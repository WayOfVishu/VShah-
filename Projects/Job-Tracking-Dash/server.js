...
// ---------------------------------------------------------------------------
// Job-Board-Scraping (PRD req. 17: separate project, one shared jobs.db).
// Only pure, dependency-free modules are imported across the project boundary.
// ---------------------------------------------------------------------------
const SCRAPER_DIR = process.env.JOB_SCRAPER_DIR || path.join(__dirname, "..", "Job-Board-Scraping");
const scraperUrl = (rel) => new URL(`file://${path.join(SCRAPER_DIR, rel).replace(/\\/g, "/")}`).href;

const { applyMigrations } = await import(scraperUrl("db/migrate.js"));
const { transition, USER_TRIGGERABLE } = await import(scraperUrl("lib/statusMachine.js"));
const { buildTailoringPrompt } = await import(scraperUrl("lib/promptBuild.js"));
const { checkTraceability } = await import(scraperUrl("lib/traceabilityCheck.js"));
const { tailorJob } = await import(scraperUrl("lib/tailorInvoke.js"));
const app = express();
...
// Applies db/migrations/*.sql from Job-Board-Scraping — the `discovered_jobs`,
...
// The status machine in Job-Board-Scraping/lib/statusMachine.js is the single
...
// req. 31: sorted by match_score then recency, archived hidden by default.
// match_score is nullable in v1 (no scoring logic ships yet), so NULLs sort
// last rather than ahead of everything.
